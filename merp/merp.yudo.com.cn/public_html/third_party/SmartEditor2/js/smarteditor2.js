...
nhn.husky.SE2M_QuickEditor_Common = jindo.$Class({
	...
	$ON_MSG_APP_READY : function() {
    	...
    	if(!this._bUseConfig){	
			this.drawDataSetting("{table:'full',img:'full',review:'full'}");
		} else {
		...
		jindo.$Ajax(self._sBaseAjaxUrl, {
			...
			onload: function(rp) {
				...
				if (!!result && !!result.text_data) {
					//self.drawDataSetting(result[result.length - 1]);
					self.drawDataSetting(result.text_data);
				} else {
					self.drawDataSetting("{table:'full',img:'full',review:'full'}");
				}
				// --[SMARTEDITORSUS-1028][SMARTEDITORSUS-1517]
			},
	...
	drawDataSetting : function(sResult){
		var oResult = {
			table : "full",
			img : "full",
			review : "full"
