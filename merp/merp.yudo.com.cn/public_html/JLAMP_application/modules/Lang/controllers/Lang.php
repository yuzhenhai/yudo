...
class Lang extends Base
{
    ...
    public function addCHN(){
        $result = DB::queryRows("select * from brpWordInfo where
                  ParentWordID = 'G2018020217591775079' and LangID = 'DEV'");
        ...
        foreach ($result[0] as $k => $v){
            $add = [
                ...
                'WordCategory'  => 'B',
                'WordText'      => array($v['WordText'],'utf-8'),
                'DCCode'        => $v['DCCode'],
                'ControlType'   => 'TextEdit',
                'LabelCaption'  => array($v['WordText'],'utf-8'),
                'EditWidth'     => '100',
                'GridColumnCaption' => array($v['WordText'],'utf-8'),
                'GridColumnWidth' => '100',
                'SetInfo1'      => '',
                'SetInfo2'      => '',
                'WordDesc'      => '',
    ...
    /**
     * 快速注册语言中文信息到数据库中，修改$wordList即可
     */
    public function addLang(){
        $wordList = [
            'YIBIDAO'=>'毅比道',
        ];
        $sql = "insert into brpWordInfo(
                     LangID
                    ,WordID
                    ,ParentWordID
                    ,BrotherWordID
        ...
        foreach ($wordList as $k => $v){
            $result = DB::queryRow("select WordID from brpWordInfo where DCCode = '%s'",[$k]);
            if(!empty($result)){
                $data['error'][] = [$k => $v];
            }else{
                $wordId = date('YmdHis',time()).rand(10000,99999);
                $langId = 'DEV';
                DB::queryRow($sql,[$langId,'W'.$wordId,'W'.$wordId,$v,$k,$k,$k]);
                $langId = 'CHN';
                DB::queryRow($sql,[$langId,'W'.$wordId,'W'.$wordId,$v,$k,$v,$v]);
                $data['success'][] = [$k => $v];
            }
        }
        $this->builder();
        Helper::responseData($data);
        $this->response();
    }
}