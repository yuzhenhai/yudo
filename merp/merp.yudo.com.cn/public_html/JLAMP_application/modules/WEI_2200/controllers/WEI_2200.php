...
class WEI_2200 extends Base {
	...
	public function lists() {
		...
		$cssPart = array(
            '<link rel="stylesheet" href="/css/WEI_2200/WEI_2200_Lists.css?v=201806080935">',
            '<link rel="stylesheet" href="/js/WEI_2100/css/layui.css">',
            '<link rel="stylesheet" href="/css/WEI_2000/mui.min.css">',
    ...
    public function changePlanFinishYn(){
        $planNo = $this->jlamp_comm->xssInput('planNo','post');
        $planFinishYn = $this->jlamp_comm->xssInput('planFinishYn','post');
        $actGubunId   = $this->jlamp_comm->xssInput('actGubunId', 'post');
        if(empty($planNo) || empty($planFinishYn)){
            $this->recall_array['data'] = 'I001';
            $this->jlamp_comm->jsonEncEnd($this->recall_array,true);
        }
        $this->load->model('Worker20_model');
        $planIsRept = $this->Worker10_model->table('TOAActRept00')->where(array('ActPlanNo' => $planNo,'CfmYn' => 1))->find();
        $planData = $this->Worker10_model->table('TOAActPlan00')->where(array('ActPlanNo' => $planNo))->find();
        if($planFinishYn == 'Y'){
            if(empty($planIsRept) || str_replace(' ','',$planData['ActGubun']) == 'OA10010100' ){
                $this->Worker20_model->table('TOAActPlan00')->where(array('ActPlanNo' => $planNo))->save(array('FinishYn','N'));
            }else{
                $this->recall_array['returnCode'] = 'B002';//已存在
            }
            $this->recall_array['returnMsg'] = 'N';
        }else{
            if(!empty($planIsRept) || str_replace(' ','',$planData['ActGubun']) == 'OA10010100' ){
                $this->Worker20_model->table('TOAActPlan00')->where(array('ActPlanNo' => $planNo))->save(array('FinishYn','Y'));
            }else{
                $this->recall_array['returnCode'] = 'B001';//不存在
            }
            $this->recall_array['returnMsg'] = 'Y';
        }
        $this->jlamp_comm->jsonEncEnd($this->recall_array,true);
    }
    //系统大分类查询
