...
var multiObject = {
    ...
    actionBind:function (arrs){
        ...
        // -- select click
        m_input.addEventListener('tap',function(e) {
            //更新全局数据
            // multiObject.init(arrs.id,arrs.vuejs,arrs.option,arrs.callback);
            //引用vue.js参数更新option下拉列表
            multiObject.upDate(_vue,_vueData,m_ul,m_input,_callback,1);
            //设定select定位，大小
            multiObject.selectTap(m_ul,m_input,_callback);
            e.stopPropagation();
        });
        ...
        // -- option click
        m_ul.addEventListener('tap', function(e) {
            multiObject.ulClose(m_ul);
        });
    },
    //设定select
    selectTap:function(m_ul,m_input){
        m_ul.style.width = m_input.offsetWidth + 'px';
        jq(m_ul).css('top', jq(m_input).position().top + m_input.offsetHeight +50+ 'px');
    ...
    optionTap:function(m_ul,m_input,_callback){
        ...
        for(var i=0 ;i<mineli.length;i++){
            mineli[i].index = i;
            mineli[i].addEventListener('tap', function(e) {
