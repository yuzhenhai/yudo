var leon = new Vue({
    ...
    mounted(){
        ...
        try {
            langCode.getWord({
                    ...
                    productNoOutDraw:'W2019060617343010385',//生产未出图
                    money:'W2019060617371554742',//金额

                    targetAndResults:'W2018013117215555371',
    ...
    methods:{
        ...
        getResults:function(){
            mui.showLoading('loading');
            var params = {}
            params.baseDate = leon.salesTargetDate;
            params.workType = 'P';
            this.isactive = 0;
            ...
            http.get(this.api.getResults,params,function (res) {
                console.log(res);
                var sumMiInvoiceForAmt = 0;
                var sumMiBillForAmt = 0;
                var sumMiReceiptForAmt = 0;
                var sumMiProductForAmt = 0;
                var sumMiWkAptForAmt = 0;
                var sumDrawMiOutForAmt = 0;
                for(var index in res.data.res){
                    res.data.res[index].titlecolor = '#2e75c4';
                    res.data.res[index].background = 'white';
                    res.data.res[index].color = '#000';
                    res.data.res[index].MiInvoiceForAmt   = parseInt(res.data.res[index].MiInvoiceForAmt/10000);
                    res.data.res[index].MiBillForAmt = parseInt(res.data.res[index].MiBillForAmt/10000);
                    res.data.res[index].MiReceiptForAmt    = parseInt(res.data.res[index].MiReceiptForAmt/10000);
                    res.data.res[index].MiProductForAmt = parseInt(res.data.res[index].MiProductForAmt/10000);
                    res.data.res[index].MiWkAptForAmt = parseInt(res.data.res[index].MiWkAptForAmt/10000);
                    res.data.res[index].DrawMiOutForAmt = parseInt(res.data.res[index].DrawMiOutForAmt/10000);
                    sumMiInvoiceForAmt += res.data.res[index].MiInvoiceForAmt;
                    sumMiBillForAmt += res.data.res[index].MiBillForAmt;
                    sumMiReceiptForAmt += res.data.res[index].MiReceiptForAmt;
                    sumMiProductForAmt += res.data.res[index].MiProductForAmt
                    sumMiWkAptForAmt += res.data.res[index].MiWkAptForAmt;
                    sumDrawMiOutForAmt += res.data.res[index].DrawMiOutForAmt;
                }
                res.data.res.push({
                    titlecolor:'#282832',
                    background:'#fff981',
                    color:'#000',
                    DeptDiv1:'Total',
                    MiInvoiceForAmt:sumMiInvoiceForAmt,
                    MiBillForAmt:sumMiBillForAmt,
                    MiReceiptForAmt:sumMiReceiptForAmt,
                    MiProductForAmt:sumMiProductForAmt,
                    MiWkAptForAmt:sumMiWkAptForAmt,
                    DrawMiOutForAmt:sumDrawMiOutForAmt,
                });
                leon.list.resultsList = res.data.res;
        ...
        buildHistogram:function (index) {
            ...
            this.view.echarts.setOption({
                ...
                legend: {
                    data:[
                        leon.lang.noInvocie,leon.lang.noBill,leon.lang.noReceipt,leon.lang.productNoInvoce,leon.lang.productNoAccept,leon.lang.productNoOutDraw
                    ]
                },
                toolbox: {
                    show : false,
                ...
                series : [{
                    name: leon.lang.money,
                    type: 'bar',
                    data: [
                        {value:leon.list.resultsList[index].MiInvoiceForAmt},
                        {value:leon.list.resultsList[index].MiBillForAmt},
                        {value:leon.list.resultsList[index].MiReceiptForAmt},
                        {value:leon.list.resultsList[index].MiProductForAmt},
                        {value:leon.list.resultsList[index].MiWkAptForAmt},
                        {value:leon.list.resultsList[index].DrawMiOutForAmt},
                    ],
                    itemStyle: {
                        normal: {
                            label: {
