...
class Quote10_model extends Multi_dbQuery {
    ...
    //.获取报价单单列表
    public function getQuoteList($quoteNo,$custNm,$startDate,$endDate,$count,$auth){
        $empId = $auth['empId'];
        $deptCd = $auth['deptCd'];
        $jobNo = $auth['jobNo'];
        switch ($auth['auth']) {
            ...
            case AUTH_E:   //个人
                ...
                break;
            case AUTH_J:   //职位
                $this->auth = " AND TSAQuot00.JobNo = '$jobNo'";
                break;
            case AUTH_D:   //部门
        ...
        $result = DB::queryRows("
			            ...
			            FROM 	    TSAQuot00
						Left Outer Join TMADept00 as DEPT With(Nolock) On TSAQuot00.DeptCd = DEPT.DeptCd
						Left Outer Join TMAEmpy00 as EMPY With(Nolock) On TSAQuot00.EmpID = EMPY.EmpID
                        ...
                        WHERE TSAQuot00.ExpClss	= '1'
                          ...
                          AND TSAQuot00.QuotDate <= '%s' $this->auth )T where id > %s order by id asc",[$quoteNo,$custNm,$startDate,$endDate,$count]);
        return $result;
