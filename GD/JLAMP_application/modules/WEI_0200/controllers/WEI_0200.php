...
class WEI_0200 extends Base {
    ...
    public function getk3Amt(){
        $date = $this->jlamp_comm->xssInput('date', 'get');
        $gubun = $this->jlamp_comm->xssInput('gubun', 'get');
        $model = new YBDAmount_model();
        $k3Res = Api::get_func('http://119.145.100.148:8701/api/getk3amount?gubun='.$gubun.'&date='.$date.'-12-31');
        $k3Res = json_decode($k3Res,true);
        switch ($gubun){
            case 'Y':
                $u8Res = $model->getYearAmt($date.'-12-31');
                break;
            case 'MS':
                $u8Res = $model->getMonthsAmt($date.'-12-31');
                break;
            case 'M':
                $u8Res = $model->getMonthAmt($date.'-12-31');
                break;
            case 'D':
                $u8Res = $model->getDayAmt($date.'-12-31');
                break;
        }
...
//        $this->jlamp_comm->jsonEncEnd('', true);
	}
