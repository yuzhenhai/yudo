...
function doSearch() {
	...
	jq.ajax({
		url: '/WEI_1100/WEI_1100/lists_prc',
		data: {
			workType: 'M',
			baseDate: baseDate
		},
							...
							jq.each(res.data.res, function(i) {
								...
								if (this.DeptDiv1_Cd) {
                                    html += '<tr onclick="setChartData(this, \'' + this.DeptDiv1 + '\', ' +
										this.TotOrderForAmt + ', ' +
										this.TotInvoiceForAmt + ', ' +
										this.TotBillForAmt + ', '+
										this.TotReceiptForAmt + ', ' +
										this.TotProductForAmt + ')">';
                                ...
                                html += '<td ' + cls + ' style="text-align: right;">' + JLAMP.common.currencyFormat(this.TotOrderForAmt, 2) + '</td>';
                                html += '<td ' + cls + ' style="text-align: right;">' + JLAMP.common.currencyFormat(this.TotInvoiceForAmt, 2) + '</td>';
                                html += '<td ' + cls + ' style="text-align: right;">' + JLAMP.common.currencyFormat(this.TotBillForAmt, 2) + '</td>';
                                html += '<td ' + cls + ' style="text-align: right;">' + JLAMP.common.currencyFormat(this.TotReceiptForAmt, 2) + '</td>';
                                html += '<td ' + cls + ' style="text-align: right;">' + JLAMP.common.currencyFormat(this.TotProductForAmt, 2) + '</td>';
                                html += '</tr>';
                            });
						} else {
        ...
        error: function(xhr, status, error) {
        	...
        	if (JLAMP.common.getDevicePlatform() === JLAMP.devicePlatform.iOS && xhr.status == 0) {
                try{
                    webkit.messageHandlers.jmobile.postMessage({fn: "callErrorPage"});
                }catch (e) {}
...
function setChartData(obj, colName, todayOrderForAmt, toDayInvoiceForAmt, toDayBillForAmt, toDayReceiptForAmt, toDayProductForAmt) {
	...
	var chartTitle1 = JLAMP.lang.getWord('W2018020109241439019');
	var chartTitle2 = JLAMP.lang.getWord('W2018020109245146068');
	var chartTitle3 = JLAMP.lang.getWord('W2018020109252733085');
	var chartTitle4 = JLAMP.lang.getWord('W2018020109261422793');
	var chartTitle5 = JLAMP.lang.getWord('W2018020109263557397');
