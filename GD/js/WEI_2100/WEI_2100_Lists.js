...
var leon = new Vue({
    ...
    filters: {
        confirmChange:function(value){
            if(value == 1 ||  value == 'Y'){
                value = '<span style="width: 60px;background:#95ce33" class="layui-badge">YES</span>';
            }
            else
            {
                value = '<span style="width: 60px;background:#a2a2a2" class="layui-badge">NO</span>';
            }
            return value;
        },
        productChange:function(value){
            if(value == 1 ||  value == 'Y'){
                value = '<span style="background:#95ce33" class="layui-badge">YES</span>';
            }
            else
            {
                value = '<span style="background:#a2a2a2" class="layui-badge">NO</span>';
            }
            return value;
        },
        productStatusChange:function(value){
            switch (value){
                case '0':
                    return '<span style="width: 60px;background: #a2a2a2" class="layui-badge">'+leon.langCode.noRelyn+'</span>';
                    break;
                case '1':
                    return '<span style="width: 60px;background: #a2a2a2" class="layui-badge">'+leon.langCode.noReceive+'</span>';
                    break;
                case '2':
                    return '<span style="width: 60px;background: #2686e1" class="layui-badge">'+leon.langCode.receive+'</span>';
                    break;
                case '3':
                    return '<span style="width:60px;background: #95ce33" class="layui-badge">'+leon.langCode.productY+'</span>';
                    break;
            }
        },
        ...
        statusChange:function(msg){
            switch (msg){
                case '0':
                    return '<span style="width: 60px;background: #a2a2a2" class="layui-badge">'+leon.langCode.apply+'</span>';
                    break;
                case '1':
                    return '<span style="width: 60px;background: #2686e1" class="layui-badge">'+leon.langCode.handle+'</span>';
                    break;
                case '2':
                    return '<span style="width: 60px;background: #95ce33" class="layui-badge">'+leon.langCode.accept+'</span>';
                    break;
                case 'A':
                    return '<span style="width: 60px;background: #2686e1" class="layui-badge">'+leon.langCode.isAdjudication+'</span>';
                    break;
            }
        },
        custStatusChange:function(status,statusId){
            switch (statusId){
                case 'MA30020010': //正常
        ...
        mf_replace:function (value) {
            return value.substr(0,10);
        },
        mf_specclass:function(value){
            var clss = '';
            switch (value){
                case '1':
                    clss = 'Apprpval';
        ...
        mf_expclass:function(value){
            var clss = '';
            switch (value){
                case '1':
                    clss = leon.langCode.exportTrust;
                    break;
                case '4':
                    clss = leon.langCode.exportUntrust;
                    break;
            }
            return clss;
        },
        mf_asconfirm:function (value) {
            if(value == 1 ||  value == 'Y'){
                value = '<div class="basic_table_div" style="width: 50px;text-align: center"><i class="layui-icon layui-icon-ok-circle" style="font-size: 20px; color: #ff9901;"></i></div>';
            }
            else
            {
                value = '<div class="basic_table_div" style="width: 50px;text-align: center"><i class="layui-icon layui-icon-ok-circle" style="font-size: 20px; color: #a39ea8;"></i></div>';
            }
            return value;
        },
        mf_switchInt:function (value) {
    ...
    methods:{
        ...
        order_query_open:function(){
            ...
            this.find_orderlist_show = true;
            // popOrderModal.showPopOrderModal(this.order_query_close,this.set_order);
            this.slide_panel_show = false;
        },
        order_query_close:function(check){
            // console.log(123);
            this.orderlist = [];
            this.slide_panel_show = true;
            this.find_orderlist_show = false;
            this.order_isloadding = false
            this.order_nodata = false;
            this.global_order_num = 0;
            mui.hideLoading()
        },
        //.从AS接受导入到AS处理
        ...
        //.AS查询筛选扩展
        showAsListScreen:function(e){
            this.asListScreen = !this.asListScreen;
        },
        //.AS查询筛选确定
        asListScreenCnf:function(){
            this.asListScreen = !this.asListScreen;
            this.query_as();
        },
        //.AS查询筛选重置
        asListScreenClear:function(){
            this.input_asusername = '';
            this.input_ascustname = '';
        },
        //客户查询界面
        ...
        spec_query_open:function(){
            ...
            popSpecModal.show(
                function () {
                    ...
                    this.ajaxfunc('/WEI_2100/spec_minute',params,'set_spec');
                }.bind(this)
            );
        },
        ...
        astableid_query_close:function(){
            this.global_item_num = 0;
            this.asitemlist = [];
            this.asitem_isloadding = false
            this.asitem_nodata = false;
            this.find_astable_show = false;
        },
        ...
        assaleslist_query_open:function(){
            popEmpyModal.show(
                function () {
                    ...
                    this.ajaxfunc_post('/WEI_2100/save_sales',params,'save_sales');
                }.bind(this)
            );
        ...
        //用户查询逻辑
        init_query_users:function(){
            this.ajaxfunc('/WEI_2100/user_prc',1,'query_users');
        },
        //订单查询逻辑
        query_order:function(){
            ...
            params.orderid = this.input_orderno;
            params.orderby = this.input_custname;
            params.ordercount = 0;
            this.ajaxfunc('/WEI_2100/order_prc',params,'query_order');
        },
        ...
        query_group:function(){
            ...
            this.ajaxfunc('/WEI_2100/group_prc',params,'query_group');
        },
        query_as_table:function(asId,check){
            ...
            http.get('/WEI_2100/as_minute_table',params,res => {
                this.dis_astablelist(res.data[0]);
            })

        },
        query_as_photo:function(){
            // mui.alert('功能测试中','YUDO');
            // mui.showLoading('loading...','div');
            // return false;
            var params = new Object();
            params.asid = this.w_asid;
            if(this.w_asid != ''){
                mui.showLoading('loading...','div');
                this.ajaxfunc('/WEI_2100/as_photo',params,'query_as_photo');
            }
        },
        query_as_sales:function(){
            ...
            if(this.w_asid != ''){
                params.asid = this.w_asid;
                this.ajaxfunc('/WEI_2100/as_sales',params,'query_as_sales');
            }
        },
        set_order:function(e){
            ...
            mui.showLoading('loading...',leon.global_title);
            var params = new Object();
            params.orderid = this.orderlist[e].OrderNo;
            // params.orderid = order.OrderNo;
            this.ajaxfunc('/WEI_2100/order_minute',params,'set_order');
        },
        set_order_minute:function(res){
            ...
            this.ajaxfunc('/WEI_2100/as_count',params,'set_ascount');
            this.slide_panel_show = true;
            // popOrderModal.hidePopOrderModal();
        ...
        refresh_as:function(asid){
            ...
            var params = new Object();
            params.asid = asid
            this.ajaxfunc('/WEI_2100/as_minute',params,'set_as');
            this.query_as_table(asid,'refresh');
        },
        refresh_order:function(orderno){
            mui.showLoading('loading...',leon.global_title);
            var params = new Object();
            params.orderid = orderno
            params.orderby = this.input_custname;
            params.ordercount = 0;
            this.ajaxfunc('/WEI_2100/order_prc',params,'refresh_order');
        },
        ...
        set_asitem:function(e){
            this.w_astable_ItemNo = this.asitemlist[e].ItemNo;
            this.w_astable_ItemNm = this.asitemlist[e].ItemNm;
            this.w_astable_ItemCd = this.asitemlist[e].ItemCd
            this.w_astable_Spec   = this.asitemlist[e].Spec;
            this.w_astable_UnitNm = this.asitemlist[e].UnitNm;
            this.w_astable_UnitId = this.asitemlist[e].UnitCd;
            this.w_astable_PreStockQty = parseInt(this.asitemlist[e].PreStockQty);
            this.astableid_query_close();
        },
        //用户查询逻辑
        set_user:function(e){
        ...
        //ajax get方法
        ajaxfunc:function (url,params,check) {
            jq.ajax({
                url: url,
                data: params,
                type: 'get',
                ...
                success: function (res) {
                    switch (check) {
                        case 'set_as':
                            ...
                            break;
                        case 'set_spec':
                            leon.dis_spec_minute(res.data);
                            break;
                        case 'set_order':
                            leon.set_order_minute(res.data);
                            break;
                        case 'set_ascount':
                            console.log(res)
                            leon.dis_as_count(res.data);
                            break;
                        case 'login_user':
                            leon.loginUserNm = res.data.username;
                            leon.loginUserId = res.data.userid;
                            leon.loginGroupId = res.data.groupid;
                            leon.loginGroupNm = res.data.groupname;
                            leon.loginUser = res.data.username;
                            leon.loginId  =  res.data.userid;
                            break;
                        case 'query_order':
                            if (res.returnCode == 'NULL') {
                                leon.orderlist = [];
                                leon.order_isloadding = false;
                                leon.order_nodata = true;
                            ...
                            leon.dis_order(res.data[0]);
                            break;
                        case 'query_as':
                            if (res.returnCode == 'NULL') {
                                leon.aslist = [];
                                leon.as_isloadding = false;
                                leon.as_nodata = true;
                                timeout = setTimeout(function () {
                                    mui.hideLoading()
                                }, 100);
                                mui.alert(leon.langCode.permissionIsNo, leon.global_title);
                                return false;
                            }
                            leon.dis_as(res.data[0]);
                            break;
                        case 'query_as_more':
                            if (res.returnCode == 'NULL') {
                                leon.as_isloadding = false;
                            ...
                            break;
                        case 'query_as_table':
                            leon.dis_astablelist(res.data[0]);
                            break;
                        case 'query_group':
                            leon.dis_transgroup(res.data[0]);
                            break;
                        case 'query_as_item':
                            leon.dis_asitemlist(res.data[0]);
                            break;
                        case 'query_as_photo':
                            mui.hideLoading();
                            ...
                            leon.dis_asphotolist(res.data[0]);
                            break;
                        case 'query_as_sales':
                            leon.dis_assaleslist(res.data[0]);
                            break;
                        case 'refresh_order':
                            mui.hideLoading()
                            if (res.returnCode == 'NULL') {
                                ...
                                leon.order_query_close();
                            }
                            leon.orderlist = res.data[0];
                            leon.set_order(0);
                            break;
                    }
                },
        ...
        //ajax syncGet
        //ajax post
        ajaxfunc_post:function (url,params,check){
            mui.showLoading('loading...',leon.global_title);
            jq.ajax({
                url: url,
                data: params,
                ...
                success: function (res) {
                    switch (check) {
                        case 'save_astable':
                            ...
                            leon.astable_minute_close();
                            break;
                        case 'save_sales':
                            leon.assales_checkbox = [];
                            if (res.returnCode == 'I828') {
                                mui.alert(leon.langCode.userIsExist, leon.global_title);
                                break;
                            }
                            leon.query_as_sales();
                            break;
                        ...
                        case 'subAdjudication':
                            if(res.returnCode == 'I451'){
                                ...
                                leon.ajaxfunc_post('/WEI_2100/as_sendSmsToUser',params,'as_sendSmsToUser');
                            }
                            break;
                        case 'unSubAdjudication':
                            if(res.returnCode == 'I452'){
                                mui.alert(leon.langCode.recordIsDoing,leon.global_title);
                                break;
                            }
                            else if(res.returnCode == 'I450'){
                                mui.alert(leon.langCode.recordIsNot,leon.global_title);
                                break;
                            }
                            else if(res.returnCode == '0'){
                                mui.alert(leon.langCode.cancelSuccess,leon.global_title);
                            }
                            break;
                        case 'save_as':
                        ...
                        case 'confirm':
                            ...
                            mui.alert(res.data, leon.global_title);
        ...
        //渲染订单列表
        dis_order:function(list){
            var nodata = 0;
            //递增加载
            if(this.global_order_num > 0){
                for(var i = 0;i<50;i++){
                ...
                if(nodata == 1){
                    this.order_isloadding = false;
                    this.order_nodata = true;
                }
                else
                {
                    this.order_isloadding = true;
                }
            }
            //首次渲染
            else
        ...
        //渲染AS编号列表
        dis_as:function(list){
            var nodata = 0;
            //递增加载
            if(this.global_as_num > 0){
                for(var i = 0;i<50;i++){
                    if(list[i] == null || list[i].length <= 0)
                    {
                        nodata = 1;
        ...
        dis_astablelist:function(list){
            this.astablelist = list;
        },
        dis_asitemlist:function(list){
            var nodata = 0;
            //递增加载
            console.log(list);
            if(this.global_item_num > 0){
                for(var i = 0;i<50;i++){
                    if(list[i] == null || list[i].length <= 0)
                    {
                        nodata = 1;
            ...
            if(nodata == 1){
                this.asitem_isloadding = false;
                this.asitem_nodata = true;
            }
            else
            {
                this.asitem_isloadding = true;
            }
            setTimeout(function(){
                mui.hideLoading()
            }, 100);
        ...
        getLoginInfo:function(){
            var params = new Object();
            this.ajaxfunc('/WEI_2100/login_user',params,'login_user');
        },
        ...
        save_as:function(){
            ...
            mui.confirm(leon.langCode.holdAsReady,leon.global_title, ['YES','NO'], function(eq) {
                if (eq.index == 0) {
                    leon.ajaxfunc_post('/WEI_2100/save_as',params,'save_as');
                }
            });
        },
        save_photo:function(){
        ...
        save_astable:function(){
            ...
            this.ajaxfunc_post('/WEI_2100/save_astable',params,'save_astable');
        },
        uploadPic:function(){
            if(this.w_asid == ''){
                mui.alert(leon.langCode.noHoldAs,leon.global_title);
        ...
        del_astable:function(e,event){
            ...
            mui.confirm(leon.langCode.deleteReady,leon.global_title, ['YES','NO'], function(eq) {
                if (eq.index == 0) {
                    ...
                    nextdom.children[1].style.webkitTransform = 'translate(0,0)';
                    leon.ajaxfunc_post('/WEI_2100/del_astable',params,'del_astable');
                }
            });
        },
        del_photo:function(e,event){
            ...
            mui.confirm(leon.langCode.deleteReady,leon.global_title, ['YES','NO'], function(eq) {
                if (eq.index == 0) {
                    elem.style.webkitTransform = 'translate(0,0)';
                    nextdom.children[1].style.webkitTransform = 'translate(0,0)';
                    leon.ajaxfunc_post('/WEI_2100/del_photo',params,'del_photo');
                }
            });
        },
        del_sales:function(e,event){
            ...
            mui.confirm(leon.langCode.deleteReady,leon.global_title, ['YES','NO'], function(eq) {
                if (eq.index == 0) {
                    elem.style.webkitTransform = 'translate(0,0)';
                    nextdom.children[1].style.webkitTransform = 'translate(0,0)';
                    leon.ajaxfunc_post('/WEI_2100/del_sales',params,'del_sales');
                }
            });
        },
        //.年月日时分秒
        ...
        subAdjudication:function(){
            ...
            params.asid = this.w_asid;
            params.asUserId = this.w_asuserid;
            this.ajaxfunc_post('/WEI_2100/subAdjudication',params,'subAdjudication');
        },
        unSubAdjudication:function(){
            ...
            var params = new Object();
            params.asid = this.w_asid;
            this.ajaxfunc_post('/WEI_2100/unSubAdjudication',params,'unSubAdjudication');
        ...
        confirm:function(choose){
            ...
            this.ajaxfunc_post('/WEI_2100/confirm',params,'confirm');
        },
        isConfirm:function(){
        ...
        unit_class:function(){
            ...
            // this.ajaxfunc('/WEI_2100/as_unit',params,'select_unit');
        },
        ...
        scroll:function(){
            jq('#mui_pushitem').scroll(function () {
                var bottom = document.getElementById('mui_pushitem').scrollHeight - document.getElementById('mui_pushitem').clientHeight - jq('#mui_pushitem').scrollTop();
                if(bottom == 0 && leon.asitemlist.length > 0){
                    leon.global_item_num = leon.global_item_num+50;
                    var params = new Object();
                    params.asitemid = leon.input_asitemid;
                    params.asitemnm = leon.input_asitemnm;
                    params.asitempage = leon.global_item_num;
                    leon.ajaxfunc('/WEI_2100/as_item',params,'query_as_item');
                }
            });
            jq('#mui_pushcust').scroll(function(){
                ...
                if(bottom == 0 && leon.custlist.length > 0){
                    leon.global_cust_num = leon.global_cust_num+50;
                    var params = new Object();
                    params.custid = leon.input_custid;
                    params.custnm = leon.input_custnm;
                    params.custcount = leon.global_cust_num;
                    leon.ajaxfunc('/WEI_2100/cust_list',params,'query_cust');
                }
            });
            jq('#mui_pushspec').scroll(function(){
                var bottom = document.getElementById('mui_pushspec').scrollHeight - document.getElementById('mui_pushspec').clientHeight - jq('#mui_pushspec').scrollTop();
                if(bottom == 0 && leon.speclist.length > 0){
                    leon.global_spec_num = leon.global_spec_num+50;
                    var params = new Object();
                    params.specid = leon.input_specid;
                    params.custnm = leon.input_speccustnm;
                    params.speccount = leon.global_spec_num;
                    leon.ajaxfunc('/WEI_2100/spec_prc',params,'query_spec');
                }
            });
            jq('#mui_pushsales').scroll(function(){
                var bottom = document.getElementById('mui_pushsales').scrollHeight - document.getElementById('mui_pushsales').clientHeight - jq('#mui_pushsales').scrollTop();
                if(bottom == 0 && leon.userslist.length > 0){
                    leon.dis_users();
                }
            });
            ...
            jq('#mui_pushorder').scroll(function(){
                ...
                if(bottom == 0 && leon.orderlist.length > 0){
                    leon.global_order_num = leon.global_order_num+50;
                    var params = new Object();
                    params.orderid = leon.input_orderno;
                    params.orderby = leon.input_custname;
                    params.ordercount = leon.global_order_num;
                    leon.ajaxfunc('/WEI_2100/order_prc',params,'query_order');
                }
            });
        },
        jq_select:function () {
    ...
    layui.use('laydate', function(){
        ...
        laydate.render({
            elem: '#assettime', //指定元素
                ...
                leon.w_assetdate = dates;
            }
        });
        laydate.render({
            elem: '#asStartDate', //指定元素
            done:function(dates){
                leon.input_asStartDate = dates;
            }
        });
        laydate.render({
            elem: '#asEndDate', //指定元素
            done:function(dates){
                leon.input_asEndDate = dates;
            }
        });
    });
...
function setQRcodeResult(content) {
    ...
    if(order_id == ''){
        mui.alert(JLAMP.lang.getWord('W2018050317441830047'),leon.global_title)
    }
    else
    {
        if(leon.global_qrclass == 'order'){
            //新增AS
            leon.clear_as();
            leon.refresh_order(order_id);
        }
        else
        {
            //查询AS
            if(popAsRecvModal.view.showPop == true){
                popAsRecvModal.model.popOrderNo = order_id;
